...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user === '') {
        ...
        if (!$error) {
            ...
            try {
                $seedUser = 'alexeychikov';
                $seedHash = password_hash('9562876', PASSWORD_DEFAULT);
                $stmt = $pdo->prepare('INSERT IGNORE INTO users (username, email, password_hash) VALUES (?, ?, ?)');
                $stmt->execute([$seedUser, 'alexeychikov@gmail.com', $seedHash]);
                $seedId = (int)$pdo->lastInsertId();
                if ($seedId) {
                    $seedListId = bin2hex(random_bytes(8));
                    $pdo->prepare('INSERT IGNORE INTO lists (id, owner_id, name) VALUES (?, ?, ?)')
                        ->execute([$seedListId, $seedId, $seedUser . "'s list"]);
                    $steps[] = ['ok', "Аккаунт «{$seedUser}» создан"];
                } else {
                    $steps[] = ['ok', "Аккаунт «{$seedUser}» уже существует — пропущен"];
                }
            } catch (PDOException $e) {
                $steps[] = ['err', 'Seed-аккаунт: ' . htmlspecialchars($e->getMessage())];
            }
        }
